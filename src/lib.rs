...
pub async fn run(addr: &str, out_dir: &str) -> Result<(), Error> {
    ...
    let first_url = urls.first().ok_or_else(|| anyhow::anyhow!("No URLs found"))?;
    ...
    serde_json::to_writer_pretty(writer, &Urls { raw, urls: urls })?;
    ...
    let prev_version = match get_version(&prev_conn).await {
        Ok(prev_version_url) => {
            extract_version_from_url(&prev_version_url)
        },
        Err(e) => {
    ...
    if current_version != prev_version {
        let update_sql_path = out_dir_path.join(format!("update-{}-to-{}.sql", prev_version, current_version));
        
        println!("Generating differential update from {} to {}", prev_version, current_version);
        ...
        if let Some(d1_version) = check_d1_version(game_type).await? {
            if d1_version == prev_version {
                ...
                println!("D1 version ({}) does not match previous version ({}), full rebuild required", d1_version, prev_version);
            }
        } else {
